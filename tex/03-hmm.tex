...
\subsubsection{One-dimensional Viterbi Algotrithm}
While back-tracing, we need access to values of matrix $B$ in decreasing order.
To reduce memory, we can use checkpointing as following:
We will compute every $k$-th row of $V$.

\subsubsection{One-dimensional Posterior Decoding}

\subsubsection{Two-dimensional Viterbi algorithm}
...
$F[i,\cdot]$ be column vector from forward
Then
\begin{align}
F[0,\cdot]^T &= I^{X_0}\\
F[i,\cdot]^T &= I^{X_0}\prod_{j=1}^i M_{X_j}
\end{align}
for all $i< n$.
\end{lemma}
...
$F[k,\cdot]^T = I^{X_0}\prod_{j=1}^k M_{X_j}$ (we assume that product of zero matrices
is identity matrix). We have 
\begin{align*}
\left(I^{X_0}\prod_{j=1}^{k+1} M_{X_j}\right)[v] &= 
\left(F[k,\cdot]^T M_{X_{k+1}}\right)[v]\\ &= \sum_{u\in V} F[k,u]
...
is associative, we can use repetitive patters to speedup calculation. To adapt
