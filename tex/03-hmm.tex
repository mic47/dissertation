...
sets of labels (or colors). Then the \firstUseOf{coloring funcion} 
...
%At first we set the initial parameters $\theta_0$. We treat the missing data as a
%random variables. We compute the expected number of occurrences of all events
%($A_{u,v}, E_{u,x}, u,v\in V,x\in\Sigma$)
%using the model $\theta_0$. We use those expectations to compute the new
%parameter set $\theta_1$ using method described above. We repeat this  
%step until the parameter set $\theta_i$  converge to local maxima (more
%precisely, algorithm stops when certain criteria is meet -- the number of
%iterations or the changes in the parameters are small enough).

%In can be proved that the likelihood of the model $\theta_i$ is higher then the
%likelihood of the model $\theta_{i+1}$ and also the $\theta_i$ converges to local
%minima \cite{Durbin1998}. This algorithm is call the Baum-Welsch algorithm,
%which uses the variant of the Forward-Backward algorithm to compute the 
%expectations of the number of occurrences of the events.
