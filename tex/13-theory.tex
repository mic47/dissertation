...
constrained by the selected guide. Example of such algorithm is the most probable ball
...
We denote HERDs gain function, described in section \ref{SECTION:HERD}, as
...
Optimizing the Viterbi algorithm with footprint $f$ as an guide can be done
...
the correct footprint normalized by the length of the correct footprint
...
most probable footprint,  then we extend this proof to theorem
...
have more choices which vertex to use next. However, when $k$ approaches $n$,
$D(n,k)$  starts to decrease, because the walks are more constrained by
...
Now let $2\leq k\leq n$. Let $v(w)$ be the number of different
...
$K_k$. However, this number includes also walks that avoid some
...
n^n$, we can store $D(m,j)$ in $O(k \cdot\mbox{polylog}(k))$ bits.
...
the numeric representation of probabilities is in constant time. Assuming that the
...
The most probable restriction problem is following: given HMM, decide if there
...
sequence. Since each of $u_i$ ($1\leq i\leq n$) has to be generated,
...
rest of the sequence: $c_1c_2\dots c_m$, and state path is admissible.
If the assignment is not satisfying, there exists clause
...
and therefore any state path using this set of states will be inadmissible.
Therefore if there is admissible state path that generates $X$, then there is
an satisfying assignment for the instance of the SAT problem.
...
In this section we show proof that the most probable footprint problem is
...
symbol $S$ and several initial blocks are generated by state $S$, one
block, say $X_i$, is generated by states $S'$, $\#$, $0$, $1$, and
$T'$ and the rest of the sequence, including the final symbol $T$, is
generated by state $T$. We will say that a state path with this
...
Any set for for sequence $X$ that have non-zero probability on sequences that
