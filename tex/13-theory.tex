...
using the selected guide.  \todo{Brona: Pridaj zmienku o Jakubovom clanku, kde
to robili a zhrnutie nasich vysledkov (experimenty, np-tazkost, formalizovanie
problemu -> z konca 5.1} In this chapter, we will focus mainly on the
complexity of the first step: finding the most probable guide. Complexity of
the second step, namely finding the annotation with highest expected gain
without guides we studied in \cite{Nanasi2010, Nanasi2010mgr}. 
...
categorized into several \firstUseOf{subtypes}\cite{} based on sequence
...
and the predicted (averaged over all samples). \todo{Asi by som to normalizoval lepsie}
...
except for annotation set specificity and sensitivity\todo{Vieme overit ze
zvolene footprinty maju nizsiu pravdepodobnost nez skutocne?}. This and
...
\ref{THEOREM::SETFIXEDTRACT}, with the difference that in the end we enumerate
only a sets of size at most $k$. During the computation we use only subsets of
states with size at most $k$ and therefore the time complexity is 
\begin{equation}
O\left(\sum_{l=0}^k{m\choose l}m^2n\right)
\end{equation}
where $m$ is the number of states, $n$ is the length of the
sequence and $k$ is the size of the restriction.
\end{proof}
...
a footprint $F$ with the probability at least $p$.\todo{Ako je to s tymto problemom a labelingom?}
\begin{figure*}
...
\todo{Zabudol si tuto verziu dokazu pre restriction}
