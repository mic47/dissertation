...
section \ref{SECTION:HERD}, in \reformulate{this problem} we are trying to label
...
\end{note}
\todo{mozno by nebolo odveci restriction premenovat na guide}
...
$R:L\to W$ a \firstUseOf{restriction function} and $R(\Lambda)$ will be
\firstUseOf{restriction} $r$ of annotation $\Lambda$.
...
decoding} we search for annotation $\Lambda$, such that following equation
holds:
...
Finding the most probable restriction can be decoupled from finding the
annotation with highest expected reward. In this chapter we will focus
\reformulate{mostly} on the complexity of the first step: finding the most
...
\begin{reformulate*}
To illustrate usefulness of the two-stage decoding, we have \reformulate{done}
...
\firstUseOf{subtypes} based on sequence similarity.  Phylogenetic tree of
...
called \abbreviation{jumping HMM}{jpHMM} and it a set of \abbreviation{profile Hidden Markov
Models}{profile HMMs}\cite{Durbin1998} connected with transitions allowing to
...
Jumping HMMs are based on profile HMMs: We start with alignment of all HIV
...
We used HERD gain function, we denote it as $f_{HERD}$, described in section
...
phase, it
constructs \firstUseOf{annotation graph}, where each path from start vertex to the
...
We have selected 
\subsection{Results}
\todo{describe problem more}
To establish measures to quantify this systematic errors, we have decided to use
two \reformulate{objects}: the \reformulate{label footprint} and the
\reformulate{label set}. While the footprint catch the whole structure of
...
The \abbreviation{set}{S} of state path $\pi$ (or labeling $\Lambda$) is the set of
