...
living organisms $X$ and $Y$. There was a ancestral sequence $P$ which evolved into
...
Similarly, during evolution from $P$ to $Y'$, two bases mutated, and two
sequences were deleted. During evolution from $Y'$ to $Y$ one base mutated, and
sequence ``CGACC'' was inserted. 

From evolutionary history described above, we can create an alignment of 
sequences $X$ and $Y$ by removing ancestral sequences, and
...
is to find that alignment, or at least alignment, that is as similar as
...
In particular
we will assume that all sequences are from a finite alphabet $\Sigma$. Specifically, the DNA alphabet
contain $4$ symbols $A,C,G,T$, and protein alphabet contains $20$ codes of amino acids.
...
want alignments that contains many columns with same or similar symbols. 
Positive gap penalty causes tendency towards alignments with many gaps,
...
frequencies of the substitutions and indels in alignments created manually by biologists. Example of such
...
We define two other matrices $M_X$ and $M_Y$ of same size as $M$. Element $M_X[i,j]$
...
Therefore we can order computation of rows of $M$ so that when we compute value $M[i,j]$, the
...
are of length at most one.  Moreover, every state generate strings of constant 
length (for example it always generates one symbol on each tape).
Formal definition of pair HMMs
...
Definition of a state path is the same as for HMMs with silent states. Restriction on
...
Given sequences $X$ and $Y$ and  state path $\pi$, we 
...
because they generate residues in only one sequence. Insert states correspond to matrices $M_X$ and
...
\subsection{Viterbi Algorithm for Pair HMMs}
...
symbol in at least one sequence. 
 Let $n=|X|$ and $m=|Y|$.
...
is aligned to a gap in $X$ between positions $i$ and $i+1$.  Posterior probabilities
...
decoding}{MPD}: the marginalized posterior probability $P'(a_i)$ is defined as
follows.
\[
P'(a_i) = \begin{cases}
...
\end{cases}
\]
The MPD finds an alignment $A$ that maximizes the product of the
...
substitution scores and gap scores. The recurrence for PD is following:
\begin{align} 
M[-1,i] &= 0, 0\leq i < m\\\
M[i,-1] &= 0, 0< i < n\\
M[i,j] &= \max
...
\subsection{Pair Hidden Markov Models with Gene Structure}
...
The total length of remaining exons have
...
Generally, DoubleScan has $54$-state GpHMM with following substructures: 
...
Decoding method of DoubleScan is the Viterbi algorithm, with stepping stone
algorithm (see Section \ref{SECTION:SSA}). They use BLASTN \cite{Altschul1990}
...
predicts gene structures for a pair of related eukaryotic organisms. SLAMs
decoding method is the Viterbi algorithm.  To reduce the running time of the VA,
SLAM align sequences within $3$ bases around alignment obtained by AVID global
...
\end{center}
\caption[HMM topology of SLAMs GpHMM]{
...
\paragraph{} \firstUseOf{TWAIN} is comparative gene-finder based on GpHMM with 
 intereresting decoding algorithm \cite{Majoros2005}: at first it
independently annotates input sequences using gene-finder TIGRscan
\cite{Majoros2004}, finding signals (splice sites, start/stop codons, \dots).
Then creates parse graphs for each sequence:
signals form vertices, which
corresponds to a state of the GpHMM. Two signals are connected with an edge
if one can follow other in a gene. The probability of an edge is determined by the
probability of the most probable state path between corresponding two states in the
TIGRscans HMM.
...
\end{center}
\caption[Topology of Pairagons generalized pair hidden Markov model.]{
...
of genes.  Pairagons HMM model consists of a simple pair HMM submodel, which
...
geometric distribution (or affine gap penalties) is that they are not realistic
...
\end{center}
\caption[An HMM with two component geometric distribution]{
...
in alignments (in the first sequence) generated by such a model have
length distribution $d(n)=(a_1p_1^{n-1}(1-p_1)+a_2p_2^{n-1}(1-p_2)), n>0$ and
$d(0) = 1-a_1 - a_2$, where $n$ is the length of the gap,
$a_1$ and $a_2$ are probabilities of entering state $I_1$ and $I_2$
...
want to generate at least one gap. Zero-length gap should be modeled by the
...
$m(0)=1-a_1-a_2$.  These two scores are different ($d(n)$ is always higher).
It is possible that the Viterbi algorithm reconstruct different
...
To construct an alignment FEAST uses the Viterbi
...
\cite{Satija2010}. GRAPes HMM  consists of two submodels: one with fast
...
alignment. They considered three types of biases associated with gaps \cite{Durbin1998}. 
By \firstUseOf{true alignment}
...
Lunter {\it et al.} studied following biases.
...
\item \firstUseOf{Gap attraction} occurs when two gaps are near each other.
In such case, merging those gaps and introducing a few mismatches might lead to
higher score. 

\item \firstUseOf{Gap annihilation} occurs when there are two gaps of the same
length, one in each sequence. Since indels are not so common, removing both
...
rate variation \cite{Lunter2008}.  They simulated  $20,000$
...
simulations models only relatively simple models of the evolution.
...
Even more reduction in memory requirements can be obtained using the Hirschberg
algorithm \cite{Hirschberg1975}. 
The idea is
following: if we want an alignment of sequences $X$ and $Y$ that has bases $X_i$
...
$j$ such that $X_{\lceil n/2\rceil}$ is aligned to $Y_j$ in the optimal alignment. This procedure takes $O(nm)$
time and $O(n+m)$ memory (alignment algorithm that keeps only one row of $M$ in memory). If $X_i$ is aligned to a gap, it finds $j$ such
that $X_i$ is aligned to gap that comes right after $X_j$.
The algorithm then uses $j$ to determine submatrices $M_1,M_2$ and finds alignments in submatrices recursively.
...
with some restriction on the scoring matrix. It divides the dynamic programming
...
block. It utilizes the $O(A+B)$ algorithm for computing row minima and maxima in
