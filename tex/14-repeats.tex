...
functions.\todo{Mohol by si bud vyhodit tieto skratky, alebo ich zacat
pouzivat}
...
\begin{comment}
Block posterior decoding function is aimed for the generalized \todo{Popisat to
ako posterior decoding, ale vahy pre dlhe emisie davame inac} This decoding
method uses stricter gain function, that is defined over generalized pair
hidden Markov models (therefore we use it on the generalized SFF or TTP). It
can be viewed as an variant of posterior decoding; instead of columns we score
emissions. If same emission exists in the true alignment, $+l$, where 
