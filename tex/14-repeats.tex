...
indices to sequences. In this scenario, alignment of length $t$ of sequences
$X=x_1x_2\dots x_n$ and $Y=y_1y_2\dots y_m$ will be represented as sequence of
tuples $(a_1, b_1), \dots (a_t, b_t)$ where $a_i\in \{0, \dots, n\}\cup\{-_0,
\dots, -_n\}$ and $b_i \in \{1, \dots, m\}\cup\{-_0, \dots, -_m\}$. Number $i$
represents $i$-th symbol in respective input sequence and $-_i$ represents gap
in the sequence before position $i$. For example $(47, -_{42})$ means that
$x_{47}$ is aligned to gap that is between $y_{41}$ and $y_{42}$.  Naturally,
indices in $a_i$ and $b_i$ have to be non-decreasing, $a_0$ has to be $0$ or
$-_{-1}$, $a_t$ is $n$ or $-_m$. The constrains for $b$ are analogous.

\todo{Spravit to tak, ze budeme mat dvojice stav/anotacia a alignment cast co
bola vygenerovana}
The \firstUseOf{state path} of an alignment is the sequence of states $s_0,
\dots, s_t$ such that $s_i$ generated pair $(a_i, b_i)$. Note that in the case
of generalized pair HMM, several consecutive states in a state annotation (such
as emissions of $R$ state in SFF and TTP models) can correspond to the same
emission. \firstUseOf{Annotation} of an state path is same as for HMMs with one.
Specially we will be interested in \firstUseOf{repeat annotation}, where
annotation function $\lambda_R(u)$ is $R$ if state is $R$ state (or was created
from $R$ state in the expanded version of the model) and $u$ otherwise (for
match, insert and delete state). Repeat annotation $\lambda_R$ is equivalent to
identity function for $3$ state HMM that does not explicitly model repeats.
The probability of a state path and annotation is defined exactly as for 
HMM that generates only one sequence.

\todo{Treba sem nejako zakomponovat aj ostatne informacie, ako napriklad 
zarovnanie, asi by to chcelo spravit nieco ako }
Similarly as with regular HMMs, we can define gain functions for annotation
$G(\Lambda, \Lambda')$ that corresponds to ``similarity'' of two alignments.
Since PHMM define probability distribution $\Lambda_T$ of the correct
annotation, we can define the expected gain of annotation given sequence $X$ and
$Y$ and gain function $G$:
...
Since we do not know the correct annotation, we search for the annotation 
$\Lambda^*$ with the highest expected gain:
...
framework. \todo{Pridat tam aj anotacie}

\paragraph{The Viterbi algorithm}

Gain $G_V(\Lambda, \Lambda_T)$ function for the Viterbi algorithms assigns $+1$
if the predicted annotation $\Lambda$ is identical to true alignment
$\Lambda_T$. Optimization algorithm was described in the section \ref{}.  The
time complexity run in time $O(nmE)$ where $E$ is the number of non-zero
transitions in the model. 

\paragraph{Posterior decoding}

In the Viterbi algorithm, the non-zero gain is awarded only if an annotation is
entirely correct. Posterior decoding gain function $G_P$ is more gradual.  It
assigns $+1$ for every correctly predicted alignment column. Alignment column
$(a_i, b_i)$ is correctly predicted if there is identical alignment column in
the true alignment. We will consider stricter version of the posterior decoding
by adding additional condition: the corresponding alignment column in the
correct alignment has to have same annotation, i.e. if $i$-th column is
equivalent to $j$-th column in the correct annotation, then $\Lambda_i$ has to
be equal to $\Lambda_{Tj}$. We use this stricter condition, because with
expanded SFF and TTP it would treat repeats as gaps despite having orthologs in
the other sequence (because our models treat such repeats independently).  For
the 3-state HMM, this stricter condition does not have effect. The running time
...
identical despite their position in the sequence. Therefore we treat column $(i,
-_j)$ to be same as $(i, -_k)$ (gaps in other sequence are treated
...
posterior probabilities, we replace probability $(i, -_j)$ with the sum of $(i,
-_l)$ of all $l$. The algorithms has also time complexity $O(nmE)$.

\paragraph{Block decoding}
\todo{Prekopat to tak, aby som nepouzival generalizovany model}

Block decoding is variant po posterior decoding 

This decoding method uses stricter gain function, that is defined over
generalized pair hidden Markov models (therefore we use it on the generalized
SFF or TTP). It can be viewed as an variant of posterior decoding; instead of
columns we score emissions. If same emission exists in the true alignment,
$+l$, where 

it will require repetitive
blocks to be correctly recognized. We split the alignment into blocks. Repeat
blocks are maximal consecutive sequence of columns that are annotated as repeat
by annotation function $\lambda_R$ and were generated by . Non-repeat blocks are other columns (each
column is one block). Non-repeat blocks are scored exactly as in the posterior
decoding. Repeat block

Alternative definition can be using generalized version of SFF or TTP. Block
decoding scores every correctly predicted emission by the number of emitted
columns (repeat submodel emit sequence independently, therefore the number of
columns is equal to the number of emitted labels). Therefore it can be seen as
an variant of posterior decoding for generalized model.  

\paragraph{Block Viterbi decoding}

\paragraph{Model and algorithm combinations}
