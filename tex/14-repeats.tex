...
\todo{reformuluj tabulku, navyse rozbi ju na viac casti a pridaj dalsie}
\begin{table*}[t]
\def\M{$^*$} % mark by a star
\def\MM{$^{**}$} % mark by two stars
\def\D{$^{\dagger}$} % mark by dagger
\def\DD{$^{\dagger\dagger}$} % mark by dagger
\def\R{$^{\diamond}$}
\def\RR{$^{\diamond\diamond}$}
\caption{Accuracy of several decoding methods on simulated data.
\M: method uses the real consensus motifs. \MM: method uses 
the real consensus motifs and intervals from the real repeat blocks.
\D: method uses intervals from the real repeat blocks.
\DD: Columns with at least one masked character are considered as repeats.
\R: Parameters for the three-state submodel 
were estimated from human-chicken
alignment. 
\RR: Parameters for SFF submodel were perturbed randomly.
}
\label{tab:sim}
\def\CC#1{\multicolumn{1}{c}{#1}} % center column
\centerline{\begin{tabular}{lr@{\quad}rr@{\quad}rr}
\hline
...
\hline
\multicolumn{6}{l}{\bf Correct intervals and consensuses}\\
SFF marginalized\M  & 3.02\% & 98.93\% & 99.64\% & 77.01\% & 76.17\% \\ 
SFF posterior\M     & 3.42\% & 98.84\% & 99.51\% & 75.91\% & 80.93\% \\
SFF block\MM        & 3.21\% & 97.70\% & 99.87\% & 80.66\% & 94.44\% \\
SFF block Viterbi\MM& 3.71\% & 98.12\% & 99.85\% & 81.75\% & 92.18\% \\
SFF Viterbi\M       & 3.94\% & 98.54\% & 99.45\% & 75.55\% & 83.47\% \\
TANTAN block\D        & 3.42\% & 60.45\% & 99.90\% & 0.36\% & 0.46\% \\
TANTAN block Viterbi\D & 3.83\% & 61.74\% & 99.88\% & 0.00\% & 0.00\% \\
\hline
\multicolumn{6}{l}{\bf Other methods}\\
Context             & 5.98\% \\
Muscle              & 5.62\% \\
3-state posterior   & 4.41\% \\
3-state posterior with masked repeats\DD & 5.03\% & 99.23\% & 74.16\% & 7.66\% & 7.24\%\\
3-state Viterbi (baseline)    & 4.78\% \\
\hline
\multicolumn{6}{l}{\bf Changed model}\\
SFF marginalized\R & 3.63\% & 96.03\% & 97.74\% &  42.70\% &  43.33\% \\ 
SFF marginalized\RR & 3.36\% & 95.99\% & 97.81\% & 40.88\% & 43.08\% \\ 
\hline
\end{tabular}}
\label{simulated_experiment}
\end{table*}
...
\end{subfigure}%
\caption{
On the X axis of all graphs is error rate of all algorithms
}
\end{center}
\end{figure}
