...
we will discuss pair alignments (alignment of two sequences). \correction{Later in this
...
Other possible solution is to use theoretical model of evolution, like
Jukes-Cantor model \cite{}, \todo{este ine priklady}

Intuition behind this scoring system are probabilistic. Assume, that we have
alignment of $X$ and $Y$ without gaps. We assume that each base in DNA evolves
independently. Therefore for every pair of residues $a,b$ we have probability
...
Therefore $S[a,b]$ is proportional to the logarithm of probability that $a$
evolves into $b$. Values of $S$ are usually multiplied by some constant and
rounded to integers to avoid use of floating point numbers. 
...
constant number of mismatches \cite{}. \firstUseOf{Space seeds} allow to
contain gaps \cite{}. \firstUseOf{Vector seeds} \cite{} and \firstUseOf{daughter
seeds} \cite{Csuros2005} are generalization of previous seed methods.
