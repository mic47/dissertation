...
will be called \firstUseOf{candidates}. Rank of cadidate $k$ is the value
...
to do it) in $O(\log n)$ time for any convex function. For some convex
...
Using this algorithm, we can compute alignment with convex gap functions in
$O(nm\log n)$ time and $O(nm)$ for convex gap functions if $H$ can be computed
in constant time.
